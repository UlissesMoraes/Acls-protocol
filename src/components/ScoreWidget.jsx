...
export default function ScoreWidget({ scoreKey, color, light, border, globalW }) {
  ...
  if (sc.type === "check") {
    ...
    total = sc.formula({...numsResolved(), ...selects});
          ...
          <div key={inp.k} style={{ marginBottom:12 }}>
            ...
            {inp.type === "select" ? (
              <select value={selects[inp.k]||inp.options[0].v}
                ...
                style={{ width:"100%", maxWidth:280, border:`1px solid ${BD}`, borderRadius:6, padding:"8px 10px", fontSize:14, fontFamily:sans, outline:"none", background:"#fff" }}>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:4 }}>
                {it.options.map(o => (
                  <label key={o.v} style={{ display:"flex", alignItems:"center", gap:8, cursor:"pointer", padding:"4px 8px", borderRadius:6,
                    ...
                    <span style={{ fontSize:11, fontWeight:700, color: val===o.v ? color : "#A0AEC0", fontFamily:sans, background:"#EDF2F7", padding:"1px 7px", borderRadius:12 }}>{o.v}</span>
                  </label>
                ))}
              </div>
            </div>
